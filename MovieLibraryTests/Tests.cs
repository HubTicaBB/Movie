...
namespace MovieLibraryTests
{
    ...
    {
        [TestMethod]
        public void Toplist_ReturnsResponseObject()
        {
            var controller = new MovieController();

            var actual = controller.Toplist();

            Assert.IsInstanceOfType(actual, typeof(ResponseObject<IEnumerable<string>>));
        }

        [TestMethod]
        public void Toplist_ReturnsHundredTitles()
        {
            var controller = new MovieController();

            var actual = controller.Toplist().Content.Count();
